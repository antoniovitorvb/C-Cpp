 ...
 *                             Forca versão 1.01                             *
 *                                                                           *
 *        Criado por Ítalo Ayres e João Martins                              *
 *                                and One More Original Title 2011           *
 *                                                                           *
 *  Programa de Código Aberto e de Distribuição e Modificação livres.        *
 *            ( Se Copiado Favor dar os devidos Créditos ).                  *
 *                                                                           *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
     ...
     Sobre o Projeto:
         ...
         comandos em C. Esse programa foi feito tentando atingir o mais alto nivel
         dos parametros requeridos da linguagem:
                *Portabilidade (prestar atenção no TENTANDO. Só funciona em windows)
                *Legibilidade (Conceito relativo, portanto nao reclamem) 
                *ehh... (enfim)
            O programa foi recheado de comentários com as mais diversas explicações
         e pode servir tradicionalmente como um guia na iniciação no aprendizado
         por apresentar apenas comandos de fácil entendimento.
            Como está organizado completamente em funções, em alguns momentos
         a legibilidade pode parecer comprometida, mas pedimos aos programadores
         que usem a Guia Lateral de Classes para sempre acompanhar o andamento
         do Programa passo-a-passo.
            Este código fonte pode ser baixado no seguinte Link: db.tt/YBc1wZF
            
            Bom Proveito.            
                         
    obs.: Uma das caracteristicas mais distintas no programa é o uso de comandos
          para "ler" a tecla pressionada sem apertar [Enter], mais informações 
          desse comando na função Tecla(); e mais informações sobre o funcionamento
          dos menus do jogo na função: Menu();
    
    obs2.: Cada função, dependendo de sua complexidade, vem antes com um comentario
    ...
    obs3.: Mais comentarios ao final do programa.
*/
